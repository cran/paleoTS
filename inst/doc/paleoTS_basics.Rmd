...
The easiest way to import your own data into `paleoTS` is through the `read.paleoTS` function, which reads a text file with four columns corresponding to the means, variances, sample sizes, and ages of samples populations (in that order). This function converts the input into an object of class `paleoTS` that is the required input for most functions. See that function's help page for options and more information.
